...
const synchronizedApis = new Set([
    'getStorageSync',
    'setStorageSync',
    'removeStorageSync',
    'clearStorageSync',
...
// 需要特殊处理的 API
const otherApis = new Set(['request']);
    ...
    apis.forEach((key) => {
        ...
        if (key === 'request') {
            taro.request = (option) => {
                ...
                const { data, header, success = noop, fail = noop } = option, rest = __rest(option, ["data", "header", "success", "fail"]);
                const req = Object.assign({ body: data, headers: header }, rest);
                const res = bn.request(req);
                const requestTask = res.then((res) => {
            ...
            taro[key] = (options) => {
                return new Promise((resolve, reject) => {
                    bn[key]
                        .apply(bn, [options])
                        .then((res) => {
                        if (options === null || options === void 0 ? void 0 : options.success) {
                            options.success(res);
                        }
                        resolve(res);
                    })
                        .catch((error) => {
                        if (options === null || options === void 0 ? void 0 : options.fail) {
                            options.fail(error);
                        }
                        reject(error);
                    })
                        .finally((res) => {
                        if (options === null || options === void 0 ? void 0 : options.complete) {
...
export { components, hostConfig, initNativeApi, otherApis, promiseApis, publicApis, synchronizedApis };
